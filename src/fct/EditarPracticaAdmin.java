...
public class EditarPracticaAdmin implements WindowListener, ActionListener {
	...
	public EditarPracticaAdmin() {
		...
		practicas = conexion.obtenerListadoPracticaAdmin(); // Método para obtener las prácticas de la base de datos
		...
		if (e.getSource().equals(btnEditar)) {
			...
			for (int i = 0; i < checkBoxes.length; i++) {
				if (checkBoxes[i].isSelected()) {
					...
					conexion.rellenarChoiceSedesAdmin(comboSedes); // Método para rellenar el combo con sedes
					...
					conexion.rellenarChoiceContactosAdmin(comboContactos); // Método para rellenar el combo con contactos
			...
			ventana.setVisible(false);
			PracticaAdmin p = new PracticaAdmin();
			p.setVisible(true);
		}
	}

	public static void main(String[] args) {
		new EditarPracticaAdmin();
	}
}
